...
- `AIMessage.continuation` 作为不透明模型值参与 Message codec 和 effect 结果持久化。恢复必须保留其精确 Provider、model ID、protocol 与递归规范化 JSON 数据；事件、请求摘要、错误和 `repr` 不得暴露其内容。Runtime 不解释或跨模型转换 continuation。
