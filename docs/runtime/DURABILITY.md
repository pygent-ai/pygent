...
- `AIMessage.continuation` 作为不透明模型值参与 Message codec 和 effect 结果持久化。恢复必须保留其精确 model key、Provider、model ID、protocol 与递归规范化 JSON 数据；事件、错误和 `repr` 不得暴露其内容，请求摘要只保存包含生产模型身份的 digest。Runtime 不解释或跨模型转换 continuation。
...
远程 pin 含非空 Provider 选项时，required capabilities 必须包含 `model.provider-options.v1`；未声明该能力的 Worker 在 placement/admission 阶段被拒绝，不能静默丢弃字段。
