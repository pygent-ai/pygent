...
历史 `AIMessage` 的不透明 Provider continuation 使用标准 Message codec 经过本地、HTTP Worker 与 durable effect 边界。Runtime 只保存和传递该值；是否回传由模型 Adapter 按 model key、Provider、model ID 与 protocol 匹配决定。
