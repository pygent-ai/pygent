...
10. **中间事件是固定协议**：reasoning、正文、ToolCall 生成、usage、attempt 与模型终态只能通过封闭的 `model.*` 事件集合输出；未知 Provider 增量必须被拒绝或显式忽略，不能伪装成完成事件。`model.completed` 只能在最终 Message、ToolCall、结构化输出和 usage 完成校验后发布。流没有可信完成信号、ToolCall 无法形成合法对象或结构化输出不匹配声明 Schema 时仍必须失败。
