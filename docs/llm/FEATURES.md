...
10. **中间事件是固定协议**：最终请求、reasoning、正文、ToolCall 生成、输出回滚、usage、attempt 与模型终态只能通过封闭的 `model.*` 事件集合输出；每个真实 Provider attempt 在 I/O 前发布有界、字段 allowlist 的 `model.request.prepared`，包含完整 provider-neutral 请求与稳定 digest，超限必须 fail closed。未知 Provider 增量必须被拒绝或显式忽略，不能伪装成完成事件。流式 idle timeout 按同一 `RetryPolicy` 重试时，必须在下一 attempt 前发布 `model.output.reset` 并清空失败 attempt 的累积输出。`model.completed` 只能在完成原因允许成功且最终 Message、ToolCall、结构化输出和 usage 全部校验后发布。流没有可信完成信号、输出因 token limit 截断、内容策略拒绝、ToolCall 无法形成合法对象或结构化输出不匹配声明 Schema 时仍必须失败；只有尚未发布正文的截断 attempt 可以按 `RetryPolicy` 重试。
