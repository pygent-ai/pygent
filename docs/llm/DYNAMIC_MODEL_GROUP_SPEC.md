...
历史 `AIMessage` 携带的 Provider continuation 作为 Message 值随 Worker 和 durable effect 传输。只有 profile 中实际进入且 Provider、model ID 与 protocol 同时匹配的模型可以回传该状态；不匹配时 Adapter 必须忽略它。
