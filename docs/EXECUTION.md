...
`ExecutionStatus` is the coarse logical state: `PENDING`, `RUNNING`, `SUCCEEDED`, `FAILED`, `CANCELLED`, or `DEADLINE_EXCEEDED`. `ExecutionPhase` explains non-terminal progress such as `SUBMITTING`, `PREPARING`, `WAITING_ADMISSION`, `STARTING`, `RUNNING`, and `FINALIZING`. The owner must bring every submitted execution to one terminal `ExecutionOutcome`; admission errors belong to that logical execution. A persistence outage is not successful finalization: preserve committed recovery facts, surface the failure through existing error channels, and complete eligible recovery through the declared ownership protocol when the required infrastructure is available. Do not invent a terminal outcome that was never committed.
...
`ModelInvoker.execute(...) -> ModelExecution` is the only model SPI. `ModelExecution` owns provider routing, retry/fallback, cancellation, events, usage, and the final `ModelProviderResponse`. Native streaming is preferred when the provider capability declares it; a non-streaming provider is projected into text/tool-call/usage/finish events. Once output is visible, a later failure is terminal except for the explicitly supported stream idle-timeout retry described below. That exception requires confirmed attempt cleanup, an existing RetryPolicy decision and `model.output.reset` before the next attempt; it does not authorize retrying arbitrary post-output failures or repeating unconfirmed side effects.

A new provider attempt may start only after the previous attempt is confirmed finished. `ExecutionOptions.deadline` is the non-extendable execution budget. Expiry stops new business work; only bounded cleanup grace may complete cancellation, resource release and atomic finalization. Grace cannot reset the business budget or turn an unconfirmed operation into success. Resource-level shutdown retains its join obligations. `RetryPolicy.attempt_idle_timeout_seconds` bounds time without Provider progress: it covers the complete wait for non-streaming calls, and is restarted for the first and every subsequent valid Provider data frame in streaming calls. Cancellation cleanup is bounded by an internal one-second grace. If cleanup cannot be confirmed, the attempt fails as `OUTCOME_UNKNOWN` with the sanitized reason `cancellation_cleanup_timeout`; retry and fallback are prohibited, and the affected client object is quarantined so no overlapping provider request can start. Quarantine is released automatically when the background task exits. Explicit caller cancellation remains cancellation, while a managed execution deadline remains `ExecutionDeadlineExceeded` at the Execution boundary. Each native Provider stream has one owner task that alone advances and closes its iterator. Deployment shutdown is stricter than request cleanup: `DefaultModelInvoker.aclose()` joins active executions and all quarantined stream owners before closing shared clients and returning.

Model events use a closed vocabulary. Every logical call emits `model.started` and exactly one of `model.completed`, `model.failed`, or `model.cancelled`. Attempts emit started plus succeeded or failed. Provider-visible reasoning uses `model.reasoning.delta`; answer text uses `model.text.delta`; streamed Tool intent uses `model.tool_call.started`, `.delta`, and `.completed`. When a confirmed stream idle timeout is retryable under the existing `RetryPolicy`, `model.output.reset` follows the failed attempt and requires consumers to discard that attempt's provisional reasoning, text, and ToolCall deltas before the next attempt starts. The accumulator is reset at the same boundary, so the final response and usage come only from the successful attempt. `model.tool_call.*` describes generation only, while `tool.*` describes authorization and execution. `model.completed` is emitted only after complete Tool arguments, structured output, final usage, and the final `AIMessage` have all validated.
