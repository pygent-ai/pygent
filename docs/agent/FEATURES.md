...
11. **标准前台 Agent 仍是组合**：`PygentAgent` 只组合标准 ReAct、前台模型、Compressor 和工具 Module。System Prompt 与 Compression Prompt 属于不可变 Agent 定义；Agent 使用上下文窗口比例和经真实 `input_tokens` 单调校准的保守估算触发压缩。Compressor 是普通 Module，只读取不带 tools 的投影 fork 并返回摘要；框架统一生成 Snapshot，只替换模型消息投影、保留 pending current 和完整已提交历史。ReAct 只接受子 model Module 显式返回的单次消息投影替换，要求 revision 精确前进并禁止该边界替换 System Prompt 或 tools。
