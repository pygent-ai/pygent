...
10. **ReAct 解释自己的 Projection Operation**：Runtime Inbox 只交付 opaque 输入；`ReActLayer` 在每次模型调用前和最终返回前解释 `react.projection.operation.v2`。操作只有 ToolResult content append、独立 UserMessage 与带 revision 的 message projection replacement；拒绝只产生稳定事件，不扩展为公开生命周期 Hook、Handler、Policy 或 Runtime 业务分支。
...
13. **注入内容统一包装**：补充上下文按 InjectionKind 使用 `<runtime-context>` 或 `<user-context>` XML 包装；内容类型与注入位置分离，该包装不改变消息角色、System Prompt 或权限。
