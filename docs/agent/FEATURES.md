...
3. **上下文自决**：Agent 明确决定 Message 是否进入 Context，以及进入的时机和顺序；框架不自动追加。内置与推荐 Agent 默认把已提交的最终 Message 加入返回 Context，调用方不得重复追加。
4. **循环有界**：ReAct 的步数、调用数、deadline 与取消行为必须有界。
5. **能力归属清晰**：模型 fallback 属于模型层，工具并发属于工具层，Agent 不复制基础设施循环。
6. **服务无状态**：会话加载和提交位于 Agent 外部，同一 Agent 可并发复用。
