...
10. **运行中投影操作属于 ReAct**：Runtime 只交付 opaque 输入；ReAct 在模型调用前及最终返回前按公开协议解释 ToolResult 内容追加、独立 UserMessage 和带 revision 的消息投影替换。拒绝产生稳定事件，不扩展为 Runtime 业务分支。operation 标识、编码和校验见 Agent SDK。
