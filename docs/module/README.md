...
1. [第一原则](FEATURES.md)
2. [调用与递推状态原则](CALL_CONTRACT_SPEC.md)
3. [SDK 使用](SDK.md)
4. 本文的详细契约
...
class RecurrentModule(Module):
    """Standard Module for computations with explicit recurrent state."""
```
...
3. 活动 scope 内的 `await child(*args, **kwargs)` 永远表示 Child；scope 外直接调用 Module 不自动创建 Root，而应明确提示使用 `invoke()`/`stream()`。
...
任意 Root Module 都可以直接执行；只有满足 Runtime 当前支持调用契约的 Module 才能接入 Binding。Binding 是服务或部署级资源治理域，不是 Module/Agent 身份；托管执行树中的原始子 Module 默认继承 Root 的 Binding。预绑定 BoundModule 保留自己的显式 Runtime/Binding，Parent Binding placement 可以为原始 Child 指定固定或自适应目标。同一 Module 定义可以接入多个隔离 Binding，多个 Root Module 也可以接入同一个 Binding。BoundModule 是“定义 + 部署治理域”的可执行句柄，不拥有会话状态；同一 Module 和 BoundModule 必须能够并发复用。若未绑定 Parent 以 direct 模式调用预绑定 Child，direct scope 会把该 Child 作为独立 managed Root 交给 Child 自己的 Runtime/Binding，等待该 Runtime 支持的结果，并在 Parent 取消时等待 Child 清理；direct Parent 本身仍不创建 Binding。

组合构造函数可以接受与所需调用签名兼容的 `ModuleDependency`。原始 Module 与 BoundModule 保持不同身份，但都能在 `forward()` 内通过 `await dependency(*args, **kwargs)` 创建结构化 Child；BoundModule 不应通过继承 Module 来混淆定义身份与部署身份。
...
- direct `invoke()`、Child 调用和 `stream().final_result()` 返回该 Module 声明的业务结果；当前 managed Runtime 继续遵守其已支持的 Message/Context recurrent contract。
...
Module 可以保存不可变配置、子 Module，以及显式 trusted 的 direct-capable 本地 adapter 稳定引用，但不得把某次调用的参数、局部结果、recurrent state、请求状态、业务会话状态或单次调用产生的连接/流、锁、队列写入 `self`。validator 拒绝 raw mutable stored state，属性冻结再拒绝 `forward()` 对 `self` 的调用态赋值。框架不会深入检查 opaque trusted adapter 的内部对象图；因此该声明是部署信任边界，adapter 只能维护共享 client、连接池、telemetry 或测试同步等生命周期状态，不能成为调用或业务状态旁路。违反该约束会使并发隔离和 durability 声明失效。可移植 ExecutionPlan 只记录 adapter 声明或资源引用，不序列化活连接。
...
Context 是可作为 RecurrentModule state 使用的不可变 Agent 状态快照，可以包含模型历史投影、有限历史视图和其他 portable 领域状态，但不是普通 Module 的强制参数，也不是完整 Execution checkpoint。`forward()` 的局部变量、普通 coroutine 等待状态、未提交事件和外部副作用不包含在任何普通返回值中。当前 Runtime 只能从自己已支持并验证的 Message/Context 边界重试或恢复，不能从任意 Python `await` 之后透明恢复。
