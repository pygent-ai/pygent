...
> 状态：设计提案。阶段 0–2 已实现（Shell 解析与身份、运行环境描述、共享 Shell 进程引擎、
> Native PowerShell/Zsh 执行、Interactive Terminal 与 `terminal_input`，direct 经本地
> ToolTask facility、managed 经 Runtime）；阶段 3 PTY 已实现、ConPTY 实验性；阶段 4 已实现
> Linux PTY + Landlock 的 workspace-write 限定（非完整沙箱）。本文冻结边界与实现范围，
> 不改变任何已发布的稳定行为。
>
...
- 会话的活状态（process、capture file、发布游标）只由 admitted executor 在任务执行内
  创建，并在其 driver 的 finally 中唯一释放；这些状态存放在显式部署资源
  `TerminalSessionStore`（`src/pygent/tool/standard/_terminal.py`）中，装配对象只持有
  该 store 引用，自身不持有任何调用级字段（总第一原则第 3 条；Tool 第一原则第 12 条）；
- `TerminalSessionStore` 的部署作用域冻结为 process-local、single event loop、
  non-durable；可被同一 event loop 中的多个 `TerminalTools` 显式共享，不跨线程、
  不跨进程，Runtime 重启后 session 不恢复；store 强制每个 `task_id` 至多一个 live
  session，重复创建报 `terminal_session_already_exists`。
...
> Interactive Terminal 的持久生命周期始终由显式 ToolTask 承载。Direct Python SDK 可以
> 通过显式本地 ToolTask facility 创建和控制 Terminal，并使用 `terminal_input`；但没有
> Runtime `ExecutionInput` / `send_input()` 外部输入通道。Managed Execution 由 Runtime
> 统一治理 Terminal 生命周期，外部输入经 ExecutionInput 由唯一消费的 Module 转成同一个
> `terminal_input` 工具调用。

Direct Execution 不为支持 Terminal 而修改既有语义：direct 缺少的是外部输入通道与
Runtime 资源治理，模型自发的工具调用本身在 direct 中仍然可用（参见 Bash 在 direct 中
由装配对象持有本地任务设施的先例）。Direct 与 Managed 共用同一条 admitted 执行路径，
Session 均由任务自身的 executor 创建。
...
- `terminal_input` 的返回值区分输出观察与执行就绪：`observation.output_quiet_seconds`
  报告传输层最后一次捕获输出字节以来的秒数；前台进程活动与 stdin 等待状态在 pipe/pty
  后端不可观测，固定为 null，phase-2 只能经由 shell/PTY job-control 内省填充，不得以
  启发式伪装执行就绪。
...
| Tool-driven interactive input (`terminal_input`) | No | Direct + Managed |
| Runtime external input (`ExecutionInput` / `send_input`) | No | Managed only |
| PTY / ConPTY | No | Yes |
...
| 1 | Native Shell Exec 支持目标 Shell。✓ 已实现（PowerShell、Zsh；Bash 沿用既有 `standard.shell.bash`） | 调用独立（连续两次调用不共享 `cwd`、变量、alias）；workspace 越界仍被拒绝；输出投影上限、截断与管道排空规则不变 |
| 2 | Interactive Terminal（Pipe 后端、受信 input 工具；direct 经本地 ToolTask facility，managed 经 Runtime）。✓ 已实现（含 `TerminalSessionStore`、有序输入流） | 查询、停止与输入指向同一 ToolTask，不重复启动命令；等待到期不改变生命周期语义；direct 没有 Runtime `send_input` 外部输入通道 |
| 3 | PTY / ConPTY 后端。✓ PTY 已实现；△ ConPTY 实验性（输出管道在部分环境仍不转发，优先 pipe） | 交互与回显能力边界在 SDK 中显式声明，不宣称与人类终端一致 |
| 4 | 进程树级隔离与 profile 声明。△ 已实现 Linux PTY + Landlock 的 workspace-write 限定（profile `"workspace"`，子进程确认后才声明）；✗ 不提供完整进程/网络/设备隔离 | 未实现完整隔离前不得宣称 full sandbox；capability 校验与实际隔离一致 |
...
2. **非 Bash Shell 的路径与编码范围**：现有 MSYS 路径归一化与 UTF-16 解码逻辑是 Bash 专属，
   新 Shell 需要各自的可验收范围。
3. **隔离 profile 的 capability 派生**：profile 已命名为 `workspace`，并在 direct PTY +
   Landlock 路径实现（子进程确认后才声明）；managed 模式下 `tool.sandbox.workspace` 的
   capability 派生与 executor 注册校验仍需与 Runtime 对齐。
4. **PTY 后端的能力边界文案**：Pipe 与 PTY/ConPTY 下的 prompt、回显、编码与信号行为差异，
   必须在 SDK 中显式声明，而不是让使用者假定与人类终端一致。
5. **工具身份粒度**：第 12 节的建议是"按 Shell 家族各自独立 `tool_id`"；若改为参数化单个
