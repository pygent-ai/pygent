...
11. **同步与 detach 对所有工具统一**：普通工具与 Agent-backed Tool 都可以声明同步等待和独立任务两种生命周期，但模型不得通过 ToolCall 参数自行提升为独立任务；生命周期只能由应用或自定义授权 Module 决定。direct execution 的独立生命周期由显式任务设施承载；Bash 装配对象可自动创建并持有本地设施，并负责关闭。原生 Python 可返回活 ToolTaskHandle，模型适配必须在输出校验与序列化之前将其转换为严格 JSON 的任务快照和结果。managed execution 中，同步 Agent-backed Tool 可以作为当前 Execution 的结构化 Child；detach 必须为该调用创建具有独立身份和 admission 的 ToolTask，默认立即返回 `ToolResult(status="detached", task=<ToolTask 公开快照>)`。应用通过 ToolSpec.wait_timeout 显式配置独立任务 admission 后有限等待；生命周期仍须应用或授权 Module 选择 detach：期限内完成则返回最终 ToolResult，否则返回 detached 结果及当前快照；等待到期不取消任务、不重新执行，也不重置任务执行预算。独立身份从 admission 起生效，观察等待的退出不转移或终结任务所有权。ToolTask 是严格 JSON、不可变的公开任务描述，不是活句柄；后续使用其稳定 `task_id` 查询、取消和取得最终 ToolResult。需要 durable recovery 时由独立 Job 承载该 ToolTask。detach 后的调用不再是 Child，但仍受声明的 Binding、资源与 capability 治理；它不得通过 `detached Child` 绕过父子取消、join、容量和终态约束。
